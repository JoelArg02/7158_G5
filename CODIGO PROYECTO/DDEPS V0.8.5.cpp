...
else
{
	...
	if (intento > MAX_INTENTOS) {
		...
		printf(CI"\t\t\t==========================================================================\n"ANSI_COLOR_RESET);
		printf(CI"\t\t\t##                                                                      ##\n");
		printf("\t\t\t##"RED"                 Se supero el numero maximo de intentos!              "CI"##\n");
		...
		switch(opc){
			...
			case 3: 
				...
				printf("\t\t\t##                                                                      ##\n");
  				...
  				printf("\t\t\t##"ANSI_COLOR_RESET"                     Se elimino de manera correcta                    "CI"##\n");
  				printf("\t\t\t==========================================================================\n"ANSI_COLOR_RESET);
    			getch();
				}else{
        		printf("\t\t\tHubo un error vuelva a intentar.\n");   
        		printf("\t\t\tRecuerda que no puedes borrar tu propio usuario!.\n");  
				getch();}
				break;
			case 4:
				system("cls");
			...
			case 5:
				...
				printf("Digita enter para salir\n");
				getch();
				exit(1);
				break;
			default:
...
void validar(char num[]){
    for (int i = 0; i < strlen(num); i++)
    {
        if(!isdigit(num[i]) ) {
			...
			printf(CI"\t\t\t==========================================================================\n"ANSI_COLOR_RESET);
			printf(CI"\t\t\t##                                                                      ##\n");
			printf("\t\t\t##"ANSI_COLOR_RESET"                        ¡SOLO SE ACEPTAN CEDULAS!                     "CI"##\n");
    ...
    printf("wtf");
        ...
        if (dec-total==j)
		{
			...
			if(fp!=NULL){
				...
				printf(CI"\n\n\n\t\t\t==========================================================================\n"ANSI_COLOR_RESET);
				printf(CI"\t\t\t##                                                                      ##\n");
				printf("\t\t\t##"ANSI_COLOR_RESET"                       USUARIO REGISTRADO CON EXITO!                  "CI"##\n");
				printf("\t\t\t##"ANSI_COLOR_RESET"                                   :)                                 "CI"##\n");
				printf("\t\t\t##"ANSI_COLOR_RESET"                     Recuerda tener tu clave en resguardo             "CI"##\n");
				printf(CI"\t\t\t##                                                                      ##\n");
				printf(CI"\t\t\t==========================================================================\n"ANSI_COLOR_RESET);
				printf("\n\t\t\t\t"ANSI_COLOR_RESET"                       Tu clave es: %s                                               \n",newpas);
				fclose(fp);
				getch();
				return;
				}
...
char nombrepaciente[30];
	...
	printf("\n\t\t\t======================================================================\n");
	printf("\t\t\t\t\nFuncion valida solo para ver resultados pre-escritos por el medico a cargo del examen realizado.\n");
	fflush(stdin);
...
void exmla(void){
    ...
    if (Fd==NULL){
		...
		printf("%d",prom);
		cal = (5 * prom)/15;
		printf("\nEl examen realizado consta de un nivel de probabilidad de cuan depresiva puede estar "RED"%d"ANSI_COLOR_RESET" ",cal);
      	printf("\nSe recomienda hacer un analisis profundo si es necesario* ");
      	printf("\nEnter para continuar con la creacion de la ficha del paciente");
      	getch();
      	regsp();
      	}
...
void regsp(void){
    	...
    	printf("\t\t\t##"ANSI_COLOR_RESET"                  SE CREARA CON EL SIGUIENTE FORMATO:                 "CI"##\n");
    	...
    	printf("\t\t\t##"ANSI_COLOR_RESET"         SI INTRODUJO MAL LA INFORMACION AVISAR AL ADMINISTRADOR      "CI"##\n");
    	printf("\t\t\t##"ANSI_COLOR_RESET"                                                                      "CI"##\n");
  		printf("\t\t\t==========================================================================\n"ANSI_COLOR_RESET);
  		...
  		if (avr == NULL){
  			do{
  			printf("\nRevisa si se introdujo bien la información, para continuar digita '1' o si no es así digita '2' para reintentar :\n");
  			fflush( stdin );
 			scanf("%d",&opc);
			switch(opc){
		case 1:
			system("cls");
 			fflush( stdin );
 			printf("Escribe aqui el resultado y la conclusión que se llego con el paciente\n");
 			printf("Recuerda agragar saltos de linea o enter mientras escribes!\n");
 			gets(textoa);
 			system("cls");
 			printf("Esto es lo que se guardara en el registro del paciente: \n");
 			puts(textoa);
 			printf("Se volvera al menu al dar enter");
 			avr = fopen(ubifin, "w+");
 			fputs(textoa, avr);
 			fclose(avr);
 			getch();
		...
		printf(CI"\t\t\t==========================================================================\n");
		printf("\t\t\t##                                                                      ##\n");
  		printf("\t\t\t##"ANSI_COLOR_RESET"                     REVISA QUE TODO ESTE CORRECTO                    "CI"##\n");
 		printf("\t\t\t##"ANSI_COLOR_RESET"                    ES POSIBLE QUE EL ARCHIVO EXISTA                  "CI"##\n");
